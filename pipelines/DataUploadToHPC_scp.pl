...
#   Description:
#   ============
#   script for picking recent file in the input directory - creating fastq files, if need be   - creating md5 files - copying (scp) relevant files to HPC - checking md5sum.
#
#*************************************************************************
#
#   Usage:
...
if($opts{h})
{
    ...
    print STDERR "\nDetails: script for picking recent file in the input directory - creating fastq files, if need be - creating md5 files - copying (scp) relevant files to HPC - checking md5sum.\n";
...
sub UploadToHPC
{
    ...
    my $dirhandle = "/home/$userid/scratch/$project";
    exit, if(system("ssh $userid\@login.hpc.cam.ac.uk \"mkdir -p $dirhandle\""));
    exit, if(system("scp $files $userid\@login.hpc.cam.ac.uk:$dirhandle"));

    print "\n Directory permissions could not be changed for: $dirhandle \n", if(system("ssh $userid\@login.hpc.cam.ac.uk \"chmod -R g+srwx $dirhandle\""));
...
sub ComputeMd5sum
{
	...
	my($path,$filename) = ($1,$2), if($file =~ /(.*)\/(.*)/);
	
	system("cd $path; md5sum $filename > ${filename}.md5");
    }
}
