...
#      die Dumper( \@splits );
      ...
      if ( $splits[0]{depth} > $MIN_DEPTH ) {

	my ($max_base, $max_score) = ("N", 0);
	foreach my $base ('A', 'C', 'G', 'T' ) {
	  ($max_base, $max_score) = ($base, $splits[0]{$base}) if ($splits[0]{$base} && $splits[0]{$base} > $max_score);
	}
#      print "$splits[0]{pos}\t$max_base\t$max_score\n";
	...
	next;
      ...
      $splits[$i + $gaps]{ depth    }++;
      $splits[$i + $gaps]{ $pseq[$i]  } += ord($pqual[$i]) - 33;
    }
  }
...
print_width($consensus_seq);

# 
# 
# 
# Kim Brugger (15 May 2012)
sub print_width {
  my ($text, $width) = @_;
  $width ||= 80;

  for(my $i=0;$i<length($text);$i+= $width) {
    print substr($text, $i, $width) . "\n";
  }
