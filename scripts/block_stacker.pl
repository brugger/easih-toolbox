...
if (1) {
  ...
  if (1) {
    ...
    b_inserts_sorted(\@stack, $features, 1);
  }
  else {
    my $counter = 0;
    while(<>) {
...
sub b_inserts_sorted {
  my ( $stack, $input_data, $sort ) = @_;
...
sub b_insert_sorted {
  ...
  verbose("algorithm :: b_insert_sorted", 1);
  ...
  verbose("data: $$data[ $START ] $$data[ $END ] <=> block: $$stack_ref[ -1 ][$START] $$stack_ref[ -1 ][$END]\n", 1);

  if ( $$data[ $START  ] == $$stack_ref[ -1 ][ $START ]  &&
       $$data[ $END    ] == $$stack_ref[ -1 ][ $END   ] ) {

    verbose("IDENTICAL BLOCK", 2);
    $identical++;
    $$data[ $SUB ] = $$stack_ref[ -1 ][$SUB];
    $$stack_ref[ -1 ][$SUB] = [$data];
  }
  elsif ( $$data[ $START  ] >= $$stack_ref[ -1 ][ $START ]  &&
	  $$data[ $END    ] <= $$stack_ref[ -1 ][ $END   ] ) {
    
    verbose("CONTAINED BLOCK", 2);
    $contained++;
    b_insert_sorted( $$stack_ref[ -1 ][$SUB], $data);
  }
...
sub stack_stats {
  my ( $stack_ref, $cutoff,  $level, $max_depth, $troughs  ) = @_;
